...
contract DonationCampaign {
    struct Donation {
        uint amount;
        uint timestamp;
        string purpose; // e.g. "Zakat", "General", or campaign-specific
    }
    ...
    function recordDonation(uint _amount, string memory _purpose) public {
        donations.push(
            Donation({
                amount: _amount,
    ...
    function getDonation(uint index)
        public
        view
        returns (uint amount, uint timestamp, string memory purpose)
    {
        Donation memory d = donations[index];
        return (d.amount, d.timestamp, d.purpose);
    }
