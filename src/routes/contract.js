...
router.get("/", (req, res) => {
  res.json({ address: factoryAddress, abi: factoryJson.abi });
});
...
router.post("/create", async (req, res) => {
  try {
    ...
    await tx.wait();
    res.json({ message: "✅ Campaign created", txHash: tx.hash });
  } catch (err) {
...
router.post("/donate", async (req, res) => {
  try {
    const { campaignAddress, amount, purpose } = req.body;
    const contract = new ethers.Contract(
      campaignAddress,
      campaignJson.abi,
      wallet
    );
    const tx = await contract.recordDonation(amount, purpose);
    await tx.wait();
    res.json({ message: "✅ Donation recorded", txHash: tx.hash });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get("/donation/:address/:index", async (req, res) => {
  try {
    ...
    const [amount, timestamp, purpose] = await contract.getDonation(
      req.params.index
    );
    res.json({
